...
final class ResendEmailConfirmationToken implements EventListenerInterface {
    public function __construct(
        private readonly MailerInterface $mailer
    ) {}

    public function handle(DomainEventInterface $event): void {
        if(!$event instanceof ConfirmationTokenResent) return;
        $host = getenv('ORIGIN_ONE');
        $token = $event->tokenValue()->value();
        $body =<<<HTML
           <!DOCTYPE html>
            <html lang="es">
                                        ...
                                        <table cellpadding="0" cellspacing="0" border="0">
                                            <tr>
                                                <td align="center" bgcolor="#0d166b" style="border-radius: 8px;">
                                                    <a href="{$host}/Auth/ConfirmAccount/confirm-account.html?confirmation={$token}"
                                                        target="_blank"
